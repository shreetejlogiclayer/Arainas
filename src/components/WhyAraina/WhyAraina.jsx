...
const WhyAraina = () => {
  ...
  const pillars = [
    {
      title: 'Comfort',
      description: "Thoughtfully positioned menstrual care designed around women's needs, offering premium softness and breathability.",
      icon: Heart,
    },
    {
      title: 'Hygiene',
      description: 'A focus on safe, responsible menstrual care with strict quality standards and touch-free manufacturing practices.',
      icon: Shield,
    },
    {
      title: 'Confidence',
      description: 'Helping women experience their periods with confidence and comfort, rather than compromise or limitations.',
      icon: Sparkles,
    },
    {
      title: 'Empowerment',
      description: "Connecting women's health with entrepreneurship, learning, and meaningful financial opportunities to rise.",
      icon: Compass,
    },
  ];
  ...
  return (
    <section
      id="why-araina"
      className="py-24 bg-gradient-to-b from-araina-white to-araina-pink/5 relative overflow-hidden"
    >
            ...
            return (
              <motion.div
                ...
                className={`relative bg-araina-white border border-araina-pink/10 rounded-2xl p-8 transition-all duration-500 overflow-hidden cursor-pointer ${
                  hoveredIdx === idx ? 'shadow-lg border-araina-pink/30 -translate-y-2' : 'shadow-sm'
                }`}
                ...
                <div className="relative z-10">
                  ...
                  <div
                    className={`inline-flex p-4 rounded-xl mb-6 transition-all duration-500 ${
                      hoveredIdx === idx ? 'bg-araina-pink text-araina-white rotate-6 scale-110' : 'bg-araina-pink/10 text-araina-pink'
                    }`}
                  >
                  ...
                  <h3
                    className={`text-lg font-bold tracking-wide mb-4 transition-colors duration-300 ${
                      hoveredIdx === idx ? 'text-araina-pink' : 'text-araina-black'
                    }`}
                  >
                    {pillar.title}
                  </h3>
                  ...
                  <div className="mt-6 w-full h-[1px] bg-araina-pink/10 overflow-hidden">
                    <div
                      className={`h-full bg-araina-pink transition-transform duration-500 origin-left ${
                        hoveredIdx === idx ? 'translate-x-0' : '-translate-x-full'
                      }`}
                    />
                  </div>
                </div>
